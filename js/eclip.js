var BadChars = "`~!@^*()_={[}]\\|:;\"',<>/?123456780qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM";
...
{
	document.getElementById("waitmessage").innerHTML = "Пожалуйста, подождите, пока кроссворд загружается...";
...
function SelectThisWord(event)
{
	...
	document.getElementById("wordinfo").innerHTML = ((CurrentWord <= LastHorizontalWord) ? "по горизонтали, " : "по вертикали, ") + WordLength[CurrentWord] + " букв.";
...
function OKClick()
{
	...
	{
		document.getElementById("worderror").innerHTML  = "Отсутствуют буквы. Это слово состоит из " + WordLength[CurrentWord] + " букв.";
		document.getElementById("worderror").style.display = "block";
		return;
	}
	if (ContainsBadChars(TheirWord))
	{
		document.getElementById("worderror").innerHTML = "Введенное вами слово содержит недопустимые символы. Пожалуйста, введите только русские буквы в поле выше.";
		document.getElementById("worderror").style.display = "block";
		return;
	}
	...
	{
		document.getElementById("worderror").innerHTML  = "Вы набрали недостаточно букв. Это слово состоит из " + WordLength[CurrentWord] + " букв.";
		document.getElementById("worderror").style.display = "block";
		return;
	}
	if (TheirWord.length > WordLength[CurrentWord])
	{
		document.getElementById("worderror").innerHTML = "Вы набрали слишком много букв. Это слово состоит из " + WordLength[CurrentWord] + " букв.";
		document.getElementById("worderror").style.display = "block";
		return;
	}
...
function CheckClick()
{
	...
	if (ErrorsFound > 0 && EmptyFound > 0)
		document.getElementById("welcomemessage").innerHTML = ErrorsFound + (ErrorsFound > 1 ? " ошибок" : " ошибка") + " и " + EmptyFound + (EmptyFound > 1 ? " неразгаданных слов" : " неразгаданное слово") + " найдено.";
	else if (ErrorsFound > 0)
		document.getElementById("welcomemessage").innerHTML = ErrorsFound + (ErrorsFound > 1 ? " ошибок" : " ошибка") + " найдено.";
	else if (EmptyFound > 0)
		document.getElementById("welcomemessage").innerHTML = "Ошибок нет, но есть " + EmptyFound + (EmptyFound > 1 ? "  неразгаданных слов." : " неразгаданное слово.");
